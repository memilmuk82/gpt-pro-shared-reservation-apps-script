...
이번 README 정비에서는 73개 테스트를 실행하지 않았습니다. 테스트는 Apps Script V8, Google Workspace 사용자 식별과 쓰기 가능한 운영 구조의 Google Sheet가 필요하며 로컬 셸에서 동일하게 실행할 테스트 러너가 없습니다. 기존 README의 `73 PASS / 0 FAIL`은 이번 작업에서 재검증된 결과가 아닙니다.
...
예약·권한·관리·통계 흐름과 Apps Script만으로 구성한 내부 업무 웹앱이라는 전시 가치는 있습니다. 그러나 식별정보가 포함된 화면 캡처, 미등록 사용자의 초기 데이터 가시성, 배포자 권한 실행과 공개 초기화·테스트 함수 때문에 현재 상태로 공개해서는 안 됩니다. 실제 식별정보나 운영 값은 이 README에 기록하지 않습니다.

공개 전에는 화면 캡처를 비식별 자료로 교체하고 Git 이력까지 검토해야 합니다. 미등록 사용자 응답을 최소화하고 공개 함수에 권한 검사를 적용하며, Settings 일괄 저장의 부분 실패를 정확히 감지·복구하도록 수정해야 합니다. 개인정보와 업무 데이터의 보존 기간, 삭제·파기, 익명화 정책도 운영 주체의 승인을 받아 마련해야 합니다.

승인된 운영 Sheet 복제본에서 73개 테스트와 미등록·일반 사용자·관리자 권한 시나리오를 다시 실행해 데이터 변경 범위와 결과를 확인해야 합니다. 이 조치 후 실제 조직 도메인·사용자·예약·Sheet와 분리된 가상 데이터 전용 전시본을 만들고 검증하면 `B. 웹앱 전환 후보`로 다시 평가할 수 있습니다.
